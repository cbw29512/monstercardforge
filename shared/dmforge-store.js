(function initializeDMForgeStore(root) {
  ...
  function normalizeStore(raw) {
    ...
    for (const collection of ['campaigns', 'magicItems', 'sessions', 'encounters', 'npcs', 'loot', 'healingRooms']) base[collection] = Array.isArray(raw[collection]) ? raw[collection] : [];
  ...
  function read() {
    try { return normalizeStore(JSON.parse(root.localStorage?.getItem(STORAGE_KEY) || 'null')); }
    catch (error) { console.error('[DMForgeStore] Failed to read shared store', error); return emptyStore(); }
  }
  ...
  function ensureCampaignIn(store, name, metadata = {}) {
    ...
    if (!campaign) {
      campaign = { id: uid('campaign'), name: safeName, createdAt: now(), updatedAt: now(), sources: [], rulesets: [] };
  ...
  function listCampaigns() { return clone(read().campaigns.sort((a, b) => a.name.localeCompare(b.name))); }
  ...
  function syncSessionConsole(legacyState) {
    ...
    return mutate((store) => {
      ...
      for (const legacyCampaign of legacyState.campaigns) {
        ...
        if (working) store.sessions.push({ id: `session-console-working-${cleanText(working.id || uid(), '', 100)}`, source: 'session-console', sourceId: cleanText(working.id || '', '', 100), campaignId: campaign.id, title: cleanText(working.title, 'Current Session', 160), date: cleanText(working.date, '', 20), status: 'working', logCount: Array.isArray(working.log) ? working.log.length : 0, combatantCount: Array.isArray(working.initiative?.combatants) ? working.initiative.combatants.length : 0, updatedAt: working.updatedAt || now() });
        for (const archived of Array.isArray(legacyCampaign.archives) ? legacyCampaign.archives : []) store.sessions.push({ id: `session-console-archive-${cleanText(archived.id || uid(), '', 100)}`, source: 'session-console', sourceId: cleanText(archived.id || '', '', 100), campaignId: campaign.id, title: cleanText(archived.title, 'Archived Session', 160), date: cleanText(archived.date, '', 20), status: 'archived', logCount: Array.isArray(archived.log) ? archived.log.length : 0, combatantCount: Array.isArray(archived.initiative?.combatants) ? archived.initiative.combatants.length : 0, updatedAt: archived.archivedAt || archived.updatedAt || now() });
      }
    });
  }
  ...
  function syncMagicItems(items) {
    ...
    return mutate((store) => {
      ...
      for (const item of items) {
        ...
        store.magicItems.push({ id: `magic-item-${cleanText(item.id || uid(), '', 100)}`, source: 'magic-items', sourceId: cleanText(item.id || '', '', 100), campaignId: campaign.id, name: cleanText(item.name, 'Unnamed Magic Item', 160), category: cleanText(item.category, 'Magic Item', 80), rarity: cleanText(item.rarity, 'Homebrew', 80), owner: cleanText(item.owner, '', 100), identification: cleanText(item.identification, 'identified', 30), stage: cleanText(item.stage, 'none', 30), ruleset: cleanText(item.ruleset, 'Homebrew', 40), updatedAt: item.updatedAt ? new Date(item.updatedAt).toISOString() : now() });
      }
    });
  }
  ...
  function syncEncounters(encounters) {
    ...
    return mutate((store) => {
      ...
      for (const encounter of encounters) {
        ...
        store.encounters.push({ id: `encounter-${cleanText(encounter.id || uid(), '', 100)}`, source: 'encounter-forge', sourceId: cleanText(encounter.id || '', '', 100), campaignId: campaign.id, name: cleanText(encounter.name, 'Untitled Encounter', 160), environment: cleanText(encounter.environment, 'Other', 60), ruleset: cleanText(encounter.ruleset, '2024', 20), difficulty: cleanText(encounter.result?.difficulty, 'Unrated', 30), rawXp: Math.max(0, Number(encounter.result?.rawXp) || 0), adjustedXp: Math.max(0, Number(encounter.result?.adjustedXp) || 0), monsterCount: Math.max(0, Number(encounter.result?.monsterCount) || 0), statBlockCount: Array.isArray(encounter.monsters) ? encounter.monsters.length : 0, updatedAt: encounter.updatedAt || now() });
      }
    });
  }

  function syncNpcs(npcs) {
    if (!Array.isArray(npcs)) return read();
    return mutate((store) => {
  ...
  function syncHealingRoom(roomCode, roomState) {
    ...
    return mutate((store) => {
      ...
      store.healingRooms.push({ id: `healing-room-${String(roomCode).toUpperCase()}`, source: 'cleric-in-a-box', roomCode: String(roomCode).toUpperCase(), campaignId: campaign.id, level: Number(roomState.level) || 1, ruleset: String(roomState.ruleset || 2014), remainingCharges: Array.isArray(roomState.charges) ? roomState.charges.filter((charge) => !charge.spent).length : 0, updatedAt: now() });
    });
  }
  ...
  function counts(campaignId) {
    ...
    return { magicItems: count('magicItems'), sessions: count('sessions'), encounters: count('encounters'), npcs: count('npcs'), loot: count('loot'), healingRooms: count('healingRooms') };
  }
